...
async function handleFormSubmit(event) {
  ...
  const quantidadePorPalete = document.getElementById(
    "quantity-per-pallet"
  ).value;
  const paletesPorCarga = document.getElementById("pallets-per-load").value;

  var response = await api.calculateFreight({
    enderecoOrigem,
    enderecoDestino,
    precoDiesel,
    kmPorLitro,
  ...
  table.innerHTML = `
    ...
    <tbody>
      <tr>
        ...
        <td>${response.precoLitro.toLocaleString("pt-BR", {
          style: "currency",
          currency: "BRL",
        })}</td>
        <td>${response.precoTotal.toLocaleString("pt-BR", {
          style: "currency",
          currency: "BRL",
        })}</td>
      </tr>
