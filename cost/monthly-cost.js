...
async function renderProductions(response) {
  ...
  document.getElementById("cost-label").innerHTML = `
    <label for="cost">Custo mensal: ${response.custoMensal.toLocaleString(
      "pt-BR",
  ...
  document.getElementById("expenses-label").innerHTML = `
    <label for="expenses">Total de despesas: ${response.totalDespesas.toLocaleString(
      "pt-BR",
      { style: "currency", currency: "BRL" }
    )}</label>
  `;
  ...
  document.getElementById("total-cost-label").innerHTML = `
    <label for="total-cost-label">Custo total das produções: ${response.custoTotalProducoes.toLocaleString(
      "pt-BR",
      { style: "currency", currency: "BRL" }
    )}</label>
  `;
