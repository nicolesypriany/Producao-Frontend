...
async function renderButtons(products) {
  ...
  products.forEach((product) => {
    ...
    const updateProduct = document.getElementById(
      `button-update-product-${product.id}`
    );

    updateProduct.addEventListener("click", () => {
      const userRole = GetUserRole();
      if (userRole == "Administrador" || userRole == "Gerente") {
        window.location.href = `update-product.html?id=${encodeURIComponent(
          product.id
        )}`;
      } else {
        showAlertError("Ação não autorizada!");
    ...
    const dialogDetails = document.getElementById(
      `dialog-details-${product.id}`
    );
    const closeDetails = document.getElementById(
      `close-details-dialog-${product.id}`
    );
