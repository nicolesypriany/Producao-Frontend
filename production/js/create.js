		...
		Array.from(rawMaterials).forEach((rawMaterial => {
			table.innerHTML += `
				<tr>
					...
					<td>
						<input type="number" name="rawMaterial-quantidade" />
					</td>
				</tr>
		`;
...
async function getSelectedRawMaterials() {
	...
	const checkboxes = document.querySelectorAll('input[name="rawMaterialSelected"]:checked');
	
	checkboxes.forEach(checkbox => {
		const row = checkbox.closest('tr');
		const id = row.querySelector('input[name="raw-material-id"]').value;
		const quantidade = row.querySelector('input[name="rawMaterial-quantidade"]').value;
...
async function handleFormSubmit(event) {
	...
	try {
		...
		await api.createProduction({ data, maquinaId, formaId, ciclos, materiasPrimas });
	}
	catch (error) {
		alert(error);
	}
